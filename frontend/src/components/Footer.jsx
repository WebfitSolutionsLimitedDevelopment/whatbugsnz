      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            ...
            <li><Link to="/top-10" className="hover:text-white">Top 10 Bugs</Link></li>
            <li><Link to="/all-bugs" className="hover:text-white">All Bugs</Link></li>
            <li><Link to="/mission" className="hover:text-white">Mission</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="text-white font-semibold text-sm mb-4 tracking-wide uppercase">Get Involved</h4>
          <ul className="space-y-2 text-sm">
            <li><Link to="/submit" className="hover:text-white">Submit a Bug</Link></li>
            <li><a href="#partner" className="hover:text-white">Partner with Us</a></li>
            <li><a href="#sponsor" className="hover:text-white">Sponsor a Problem Report</a></li>
            <li><a href="#founders" className="hover:text-white">Join the Founder List</a></li>
          </ul>
        </div>
      </div>
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
          <p className="text-xs text-white/40">
            © {new Date().getFullYear()} What Bugs NZ? — Insights are directional signals, not official statistics.
          </p>
          <p className="text-xs text-white/40">Made in Aotearoa.</p>
        </div>
      </div>
    </footer>
  );
